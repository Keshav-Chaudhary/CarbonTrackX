...
import { EMISSION_FACTORS, getFactor } from "@/lib/emissions/factors";
...
describe("emission factors", () => {
  ...
  it("resolves known factors and rejects unknown ones", () => {
    expect(getFactor("car_petrol")?.category).toBe("transport");
...
describe("computeActivities", () => {
  it("drops invalid activities instead of throwing", () => {
    const activities: Activity[] = [
      ...
      { id: "3", factorId: "train", quantity: -5, date: "2026-01-01" },
    ];
...
describe("summarize", () => {
  const activities: Activity[] = [
    { id: "1", factorId: "car_petrol", quantity: 10, date: "2026-01-01" }, // 1.7 transport
    { id: "2", factorId: "train", quantity: 100, date: "2026-01-02" }, // 3.5 transport
    { id: "3", factorId: "electricity", quantity: 5, date: "2026-01-02" }, // 2.0 energy
    { id: "4", factorId: "meal_beef", quantity: 2, date: "2026-01-03" }, // 13.2 diet
  ];

  it("totals across categories", () => {
    const summary = summarize(activities);
    expect(summary.byCategory.transport).toBe(5.2);
    expect(summary.byCategory.energy).toBe(2.0);
    expect(summary.byCategory.diet).toBe(13.2);
    expect(summary.byCategory.shopping).toBe(0);
    expect(summary.totalKg).toBe(20.4);
...
describe("dailyTotals", () => {
  it("groups by date and sorts ascending", () => {
    const totals = dailyTotals([
      { id: "1", factorId: "car_petrol", quantity: 10, date: "2026-01-02" },
      { id: "2", factorId: "train", quantity: 100, date: "2026-01-01" },
      { id: "3", factorId: "car_petrol", quantity: 10, date: "2026-01-02" },
    ]);
    expect(totals).toEqual([
      { date: "2026-01-01", kg: 3.5 },
      { date: "2026-01-02", kg: 3.4 },
    ]);
  });
